...
parser = argparse.ArgumentParser(description='test all train/test compression pairs and skip completed ones')
...
compression_levels = ['original', 'CRF18', 'CRF23', 'CRF28', 'CRF35', 'CRF51']
...
def load_existing_results(summary_csv):
    ...
    with open(summary_csv, 'r', newline='') as f:
        ...
        for row in reader:
            parsed = {
                'train_compression': row['train_compression'],
                'test_compression': row['test_compression'],
...
def evaluate_model_on_dataset(model_path, test_dataset, test_num_each, train_compression, test_compression):
    ...
    print(
        '[train={} test={}] loss={:.4f} acc={:.4f} macro_f1={:.4f}'.format(
            train_compression, test_compression, test_average_loss, acc, f1_macro
        ),
        flush=True
    )
    return result
...
def write_macro_f1_matrix_csv(results, compressions, out_path):
    matrix = {train_c: {test_c: '' for test_c in compressions} for train_c in compressions}
    for row in results:
        matrix[row['train_compression']][row['test_compression']] = row['f1_macro']
...
def main():
    ...
    existing_results = load_existing_results(summary_csv)
    existing_pairs = set((r['train_compression'], r['test_compression']) for r in existing_results)
    ...
    for train_compression in compression_levels:
        ...
        if model_path is None:
            print(f"Skipping {train_compression}: no model found", flush=True)
            continue

        print(f'Using model for {train_compression}: {model_path}', flush=True)

        for test_compression in compression_levels:
            pair = (train_compression, test_compression)

            if pair in existing_pairs:
                print(f"Skipping already-done pair: train={train_compression}, test={test_compression}", flush=True)
                continue

            data_path = os.path.join(data_dir, f'train_val_test_paths_labels_{test_compression}.pkl')
            if not os.path.exists(data_path):
                print(f"Skipping {test_compression}: missing dataset {data_path}", flush=True)
                continue
            ...
            write_accuracy_matrix_csv(all_results, compression_levels,
                                      os.path.join(save_dir, 'cross_compression_accuracy_matrix.csv'))
            write_macro_f1_matrix_csv(all_results, compression_levels,
                                      os.path.join(save_dir, 'cross_compression_macro_f1_matrix.csv'))

    print('Finished testing all missing train/test compression pairs.', flush=True)
