...
parser = argparse.ArgumentParser(description='lstm training across compression levels')
...
parser.add_argument('--pretrained', default=1, type=int, help='1 to use ImageNet pretrained ResNet50, 0 otherwise')
parser.add_argument('--data_dir', default='/home/ahallur1/DL/Vani/MTRCNet-CL', type=str,
                    help='directory containing train_val_test_paths_labels_<compression>.pkl')
...
print('base save dir   : {}'.format(base_save_dir), flush=True)
print('use pretrained  : {}'.format(use_pretrained), flush=True)
print('data dir        : {}'.format(data_dir), flush=True)
print('compressions    : {}'.format(compression_levels), flush=True)
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each, save_dir, compression_level):
    ...
    print('compression level  : {}'.format(compression_level), flush=True)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            if batch_idx % 100 == 0:
                print(f"[{compression_level}] epoch {epoch} train batch {batch_idx}/{len(train_loader)}", flush=True)
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(val_loader):
                if batch_idx % 100 == 0:
                    print(f"[{compression_level}] epoch {epoch} val batch {batch_idx}/{len(val_loader)}", flush=True)
        ...
        ckpt_epoch_path = os.path.join(save_dir, f'{compression_level}_checkpoint_epoch_{epoch}.pth')
        npy_epoch_path = os.path.join(save_dir, f'{compression_level}_checkpoint_epoch_{epoch}.npy')
    ...
    print('[{}] best accuracy: {:.4f} cor train accu: {:.4f}'.format(
        compression_level, best_val_accuracy, correspond_train_acc), flush=True)
...
def main():
    for compression_level in compression_levels:
        print('\n' + '=' * 100, flush=True)
        print('Starting training for compression level: {}'.format(compression_level), flush=True)

        data_path = os.path.join(data_dir, f'train_val_test_paths_labels_{compression_level}.pkl')
        if not os.path.exists(data_path):
            print('Skipping {} because file not found: {}'.format(compression_level, data_path), flush=True)
            continue

        compression_save_dir = os.path.join(base_save_dir, compression_level)
        os.makedirs(compression_save_dir, exist_ok=True)

        train_dataset, train_num_each, val_dataset, val_num_each, _, _ = get_data(data_path)
        ...
        train_model(
            ...
            val_dataset,
            val_num_each,
            compression_save_dir,
            compression_level
        )

        print('Finished training for {}'.format(compression_level), flush=True)

    print('All compression-level training runs complete.', flush=True)
