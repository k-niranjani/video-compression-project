...
parser.add_argument('-c', '--crop', default=1, type=int, help='0 rand, 1 cent, 5 five_crop, 10 ten_crop')
...
class resnet_lstm(torch.nn.Module):
    def __init__(self):
        ...
        # pretrained=False because we are loading the trained checkpoint afterward
...
def load_checkpoint(model, checkpoint_path, device):
    ...
    try:
        ...
        # Handle module./non-module mismatch automatically
        new_checkpoint = {}
...
def test_model(test_dataset, test_num_each):
    ...
    print('type of all_preds:', type(all_preds), flush=True)
    print('leng of all preds:', len(all_preds), flush=True)
    ...
    print(f"saved predictions to {pred_name}", flush=True)
