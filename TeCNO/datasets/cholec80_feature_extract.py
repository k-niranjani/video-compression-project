...
class Cholec80FeatureExtract:
    def __init__(self, hparams):
        ...
        # data_root should point to:
        # E:/VC/cholec80_processed_1fps/videos
        ...
        # Read corrected 1 fps dataframe
        self.df["all"] = pd.read_pickle(
            self.cholec_root_dir / "dataframes" / "cholec80_1fps.pkl"
        )

        self.df["all"] = self.df["all"].dropna().reset_index(drop=True)
        self.df["all"] = self.df["all"].drop_duplicates(subset=["image_path"]).reset_index(drop=True)

        assert self.df["all"].isnull().sum().sum() == 0, "Dataframe contains nan elements"

        print("Total unique frames in dataframe:", len(self.df["all"]))
        print("Unique videos in dataframe:", self.df["all"]["video_idx"].nunique())

        # 40 / 8 / 32 split over 80 videos
        self.vids_for_training = list(range(1, 41))   # 1-40
        self.vids_for_val = list(range(41, 49))       # 41-48
        self.vids_for_test = list(range(49, 81))      # 49-80

        print("Train videos:", len(self.vids_for_training))
        print("Val videos:", len(self.vids_for_val))
        print("Test videos:", len(self.vids_for_test))

        self.df["train"] = self.df["all"][
            self.df["all"]["video_idx"].isin(self.vids_for_training)
        ]
        self.df["val"] = self.df["all"][
            self.df["all"]["video_idx"].isin(self.vids_for_val)
        ]
        self.df["test"] = self.df["all"][
            self.df["all"]["video_idx"].isin(self.vids_for_test)
        ]

        if hparams.test_extract:
            print("test extract enabled. Test will be used to extract the videos (testset = defined test split)")

        print(f"Train frames: {len(self.df['train'])}")
        print(f"Val frames:   {len(self.df['val'])}")
        print(f"Test frames:  {len(self.df['test'])}")
        ...
        if self.dataset_mode == "img_multilabel":
            ...
            # IMPORTANT: keep original row index for export
    ...
    def median_frequency_weights(self, file_list):
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        ...
        cholec80_specific_args.add_argument("--fps_sampling", type=float, default=1)
        cholec80_specific_args.add_argument("--fps_sampling_test", type=float, default=1)
...
class Dataset_from_Dataframe_video_based(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = torch.tensor(img_list[self.label_col].tolist(), dtype=torch.int)
        f_video = self.load_cholec_video(img_list)

        if self.transform:
            for i in range(len(f_video)):
                videos_x[i] = self.transform(image=f_video[i], mask=None)["image"]
        ...
        assert videos_x.shape[0] > 0, f"no video returned shape: {videos_x.shape[0]}"
