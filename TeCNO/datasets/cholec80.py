...
class Cholec80:
    def __init__(self, hparams):
        ...
        # force integer naming so 1.0 -> 1
        self.features_per_seconds = int(hparams.features_per_seconds)

        hparams.factor_sampling = int(25 / hparams.features_subsampling)
        print(
            f"Subsampling features: 25features_ps --> {hparams.features_subsampling}features_ps "
            f"(factor: {hparams.factor_sampling})"
        )

        fps_folder = f"{self.features_per_seconds}fps"
        fps_suffix = f"{self.features_per_seconds}fps"

        self.data_p = {}
        self.data_p["train"] = [
            f"{fps_folder}/video_{i:02d}_{fps_suffix}.pkl"
            for i in range(1, 41)
        ]
        self.data_p["val"] = [
            f"{fps_folder}/video_{i:02d}_{fps_suffix}.pkl"
        ...
        self.weights["train_log"] = [1.25, 0.5, 1.0, 0.25, 1.25, 1.0, 1.5]

        self.data = {}
        for split in ["train", "val", "test"]:
            self.data[split] = Cholec80Helper(
                hparams,
                self.data_p[split],
                dataset_split=split,
            )
        ...
        print(
            f"train size: {len(self.data['train'])} - "
            f"val size: {len(self.data['val'])} - "
            f"test size: {len(self.data['test'])}"
        )
